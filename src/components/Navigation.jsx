...
const Navigation = () => {
    ...
    const navClass = `fixed top-0 w-full z-50 transition-all duration-300 ${scrolled ? 'bg-[#FFFFFF]/95 backdrop-blur-md shadow-sm py-2' : 'bg-[#FFFFFF]/95 backdrop-blur-md shadow-sm py-4'}`;
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            <div className="flex justify-between items-center">
                ...
                <Link to="/" className="flex items-center gap-3 group">
                    <img src="https://horizons-cdn.hostinger.com/85e3d67c-81c2-44f6-84ab-85e62ff61b1d/pdp_logo-1-1-iIEZ5.png" alt="Prasad Dairy Logo" className="h-16 w-auto object-contain" />
                </Link>

                {/* Desktop Navigation */}
                <div className="hidden md:flex items-center space-x-8">
                    {navItems.map(item => <Link key={item.path} to={item.path} className={`text-sm font-bold tracking-widest uppercase transition-colors duration-300 hover:text-[#0065B3] ${isActive(item.path) ? 'text-[#0065B3]' : textClass}`}>
                        {item.name}
                    </Link>)}
                ...
                {/* Mobile Menu Button */}
                <Button variant="ghost" size="icon" className={`md:hidden ${textClass} hover:bg-black/5`} onClick={() => setIsOpen(!isOpen)}>
        ...
        <AnimatePresence>
            {isOpen && <motion.div initial={{
                ...
                <div className="px-4 pt-2 pb-4 space-y-1">
                    {navItems.map(item => <Link key={item.path} to={item.path} onClick={() => setIsOpen(false)} className={`block px-4 py-3 text-xs font-bold uppercase tracking-widest transition-all duration-300 ${isActive(item.path) ? 'text-[#0065B3] bg-[#0065B3]/10' : 'text-[#263238] hover:bg-[#0065B3]/10 hover:text-[#0065B3]'}`}>
                        {item.name}
                    </Link>)}
